...
2. Sistem mengevaluasi sisa saldo pada masing-masing amplop berdasarkan perilaku rollover masing-masing:
   ...
   - **Transfer ke Tabungan**: Sisa saldo dipindahkan ke amplop khusus kategori Tabungan/Investasi.
...
| **Unhappy Path (Data Orphaned)**   | Pengguna sempat membuat transaksi di household-nya sendiri sebelum bergabung ke household pasangan $\rightarrow$ Transaksi lama tersebut tertinggal di database dengan household lama yang ditinggalkan.                                                          | **Solusi**: Sistem membatasi menu "Gabung Rumah Tangga" hanya jika pengguna baru belum mencatat transaksi apa pun di household-nya sendiri. Jika sudah ada data transaksi, sistem menampilkan peringatan/konfirmasi bahwa data lama akan ditinggalkan atau tidak dapat digabungkan secara otomatis. |
...
1. **Validasi Sebelum Gabung Rumah Tangga**: Menambahkan filter backend pada `/users/join-household` untuk mencegah penggabungan jika pengguna memiliki transaksi aktif yang belum diselesaikan/dihapus secara sadar di rumah tangga lamanya.
2. **Riwayat Rollover Terpusat**: Menyimpan log khusus rollover yang mencatat berapa sisa dana yang di-reset, di-rollover, atau ditransfer ke tabungan di setiap akhir bulan sebagai bahan laporan audit tahunan pengguna.
3. **Mekanisme Sinkronisasi Offline**: Menambahkan antrean transaksi di local storage (IndexedDB) di sisi client PWA agar pengguna tetap bisa mencatat pengeluaran saat tidak ada sinyal internet, dan otomatis melakukan sinkronisasi (_sync back_) saat koneksi terdeteksi kembali.
