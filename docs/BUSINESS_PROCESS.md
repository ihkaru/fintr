...
- **Naikkan Anggaran Sekarang**: Menampilkan dialog konfirmasi pra-aksi pembanding nominal lama vs nominal baru beserta penjelasan jujur bahwa sisa saldo akan menjadi Rp 0 demi konsistensi dengan Skenario B. Setelah disetujui, sistem secara otomatis meningkatkan limit alokasi anggaran (`allocatedAmount`) amplop tersebut di basis data sebesar selisih kekurangannya (nominal transaksi dikurangi sisa saldo saat ini), kemudian langsung melanjutkan penyimpanan transaksi secara otomatis.

### 📑 E. Skenario Penanganan Unhappy Path OCR & Share Target
...
| **Rincian Item Terdeteksi (Split Mode)**         | AI Gemini berhasil mengekstrak rincian barang belanjaan individu (`items`). Sistem memunculkan banner rujukan untuk membagi transaksi. Klik pada banner langsung mengisi form ke mode Split Kategori secara otomatis sesuai item.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Mempermudah pemilahan barang belanjaan bulanan ke amplop masing-masing tanpa perlu mengetik ulang nominal satu per satu secara manual.                                                                                                                                                                                                                                                                                                                                                              |

### 📋 F. Known Limitations & Manual Workarounds
